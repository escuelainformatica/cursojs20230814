...
function enviar() {
    productoWebService.insertar(producto)
        .then((response) => {
            producto.value = {
                name: "", description: "", price: 0, quantity: 0 // asignamos el producto como vacio.
            };
            router.push('/producto/listar'); // redireccionar a listado
        })
        .catch((error) => {
            alerta.value = error;
            console.log(error);
